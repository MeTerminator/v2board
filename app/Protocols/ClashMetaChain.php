...
class ClashMetaChain
{
    public $flag = 'metachain';
    private $servers;
    private $user;
    ...
    public function handle()
    {
        ...
        $defaultConfig = base_path() . '/resources/rules/chain.clash.yaml';
        ...
        $config['proxy-groups'] = [
            ...
            // 转发模式的落地组：调用已被注入拨号代理的 provider2
            [
                'name' => '🎯 链式落地',
                'type' => 'select',
                'use' => ['chain_fallback_nodes'] 
            ],
            // 转发模式的前置组：调用普通的纯节点 provider1
            [
                'name' => '🛡️ 链式前置',
                'type' => 'select',
                'use' => ['chain_front_nodes']
            ],
            // 纯直连模式组：同样调用普通的纯节点 provider1
            [
                'name' => '✈️ 直连模式',
                'type' => 'select',
                'use' => ['chain_front_nodes']
        ...
        $config['proxy-providers'] = [
            ...
            // 落地池：利用 override 强行把所有节点嵌套在【🛡️ 链式前置】组后面
            'chain_fallback_nodes' => [
                'type' => 'inline',
                'override' => [
                    'dialer-proxy' => '🛡️ 链式前置'
                ],
        ...
        // 因为节点都在 provider 里面了，把顶层的 proxies 强制置空，防止干扰
        $config['proxies'] = [];
